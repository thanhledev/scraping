...
namespace DataTypes
{
    public class BongDaVnSpawner : ISpawning
    {                
        ...
        public void CreateSearchPagesLiveFeed(ArticleSource source, int depth, SourceCategory chosenCategories, ref List<string> searchPages)
        {
            ...
            pattern = "http://" + chosenCategories.SourceHost + "/" + pattern + "/Index.aspx";

            for (int j = 1; j <= depth; j++)
                result.Add(pattern + "?Page_ID=" + j);
